...
export class BlockItem  { 
  ...
  constructor (data, dom, parent) { 
  	...
  	this.isActive = parent.rowindex == 0  ? true : false;
  ...
  drawGroup ( ) { 
  	...
  	this.domElement.selectAll ('#' + this.data.name)
		...
		//console.log (this.groupdata2)

		// add up filteres one at a time 
		this.groupdata2.forEach ((d, i) => { 
				//console.log ('fiter = ', d.filter)
				let totalWidth = 0; 
				if (i>0) { 
					 let array = this.groupdata2.slice (0, i);
					 array = array.map ( n => n.filter); // get the filter elements from the array 
					 // add the number of items together 
					 let total =  array.reduce((total, currentArray) => total + currentArray.length, 0);
		...
		//console.log ('--------------------')
				...
				this.domElement.selectAll ('#' + this.data.name + '_group')
						...
						.attr ('width', d => d.filter.length * 5)
						.attr ('height', 10)
						.attr ('x', (d, i) => { 
								//console.log ('data ', d)
							let totalWidth = 0;
								if (i>0) {
									 ...
									 let array = this.groupdata2.slice (0, i);
									 array = array.map ( n => n.filter)
									 //console.log ('array =', array)
									 // add the number of items together 
									 totalWidth =  array.reduce((total, currentArray) => total + currentArray.length *5, 0);
									 //console.log ('total = ', total)
								
													
									// const totalWidth2 = this.groupdata2.reduce((result, item, index) => {
									// 					//console.log ('item = ', item)
									// 				  const filterLength = item.filter.length;
									// 				  const previousCount = index > 0 ? result[index - 1] : 0;
									// 				  const totalCount = previousCount + filterLength;
									...
									//totalWidth = this.groupdata.slice (0, i).reduce ((acc, cur) => acc + cur.length*5, 0)
									//prevW = nodeData[0].slice(0, i).reduce((acc, cur) => acc + cur.w, 0); // accumator (add up all the widths )
								}
								return  i + totalWidth  + this.loc.x + 120;
							})
