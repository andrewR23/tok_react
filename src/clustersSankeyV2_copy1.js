  ...
  clickBlockItem (obj, e, d) { 
  		...
  		// --- clickable when block 'isActive' OR item 'isSelected'
			if (this.isActive ||  isSelected == 'true' )  {
					...
					_root.addToSelection_Sankey (selection, this)
...
// -- draw links between blocks -- 
  ...
  createLinks () { 
  	// -- LINK DATA -- 
		...
		// -- perhaps look for matches in each groupdata group (visible) /? ?? ** 


		let nextRow = this.parent.rowindex + 1; 
			// get blocks in the next row -- // 
  	 _root.rows[nextRow].blocks.forEach ((targetblock, i) => { 
         let filter = targetblock.nodes_base.filter (n => this.nodes_base.includes(n)) // find matches in BASE---  // 
         ...
         if (filter.length > 0) {    
         		...
         		//------------------------------//
         		//------------------------------//
         		// // -- which group are the items in ? 
         		// const source_group = this.groupdata.find(obj =>
  					// 	Object.values(obj.base).some(value => filter.includes(value))
						// );
						// const target_group = targetblock.groupdata.find(obj =>
  					// 	Object.values(obj.base).some(value => filter.includes(value))
						// );
						// // -- add link to one of the three target places: selected / unselected / flow  -- //
						// let groupToAddTo = this.grouplinks.find (item => item.name == target_group.name );
						// // -- push link into one of the groups -- do we need filter count (?)
					  // let link = { source:this, target:targetblock, count:filter.length }
						// groupToAddTo.links.push (link);
  	...
  	// console.log ('---------------------------------')
  	// console.log ('result2 = ', this.data.name, '  : ', this.links)


  	// // --DRAW V1 
		  // 	let links = this.grouplinks[1].links;
		  ...
		  //   .x (d=> { 
		  //     //console.log ("dx = ", d) // 
		  //     return 100; //d.data.x + 120
		  //   })
		  //   .y (d=> { 
		  //    // console.log ('dy = ', d.parent.loc.y)
		  //     return 100; //d.parent.loc.y
		  //   })

			// 		// Draw the links inside sankey group
			// 		this.domElement.selectAll("path")
			// 		  .data(links)
			// 		  .enter()
			// 		  .append("path")
		...
		// // -- DRAW V2 drawing an arrray of links -- from AI -- //
				...
				linkGroups.each((d, i) =>  {
				  ...
				  links.forEach (l=> { 
				  			// -- source loc
				  			let sx = l.source.loc.x;
				  			let sxoff = l.source.groupdata[i].xoff;
				  			let sy = l.source.loc.y

				  			// -- target loc 
				  			let tx = l.target.loc.x;
				  			let txoff = l.target.groupdata[i].xoff;
				  			let ty = l.target.loc.y

				  			// -- 
				  		 	console.log ('source x ',  sx + sxoff, ' y ', sy)
				  		 	console.log ('target y ',  tx + txoff, ' y ', ty)
				  })
						...
						//   var linksSelection = d3.select(this)
						//     .selectAll("path")
						//     .data(links)
						//     .enter()
						//     .append("path")
		...
		// //-- V3 giving each link a groupType flag and using this to draw -- // 
 ...
 // -- end of 'createLinks( )' -- //
  ...
  drawLinks ( ) { 
  	...
  	console.log ('draw links'); // create links to the next group down.. 
  }
