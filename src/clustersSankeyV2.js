...
export class BlockClass  { 
	...
	constructor (name, domItem, groupdata, loc, i) { 
				//this.svg = domItem;
				this.groupDOM =  domItem.append ('g').attr ('id', 'blockGroup'); // the element to add clusters
				...
				this.data_root.forEach (d => { 
					let block = new BlockItem (d, this.groupDOM, this); // data - dom - parent 
				...
				// -- ADD TEXT HEADINGS headings to block-- / 
  ...
  // --------------// // FROM AI 
    updateVariableTWEEN() {
	    ...
	    var tween = (variableToUpdate) => {
	      var i = d3.interpolate(this.initialValue, this.targetValue);
	      return function (t) {
	        variableToUpdate = i(t); // Update the variable directly
	      };
	    };

		var tween = createTween(0, 400);

    d3.transition()
      .duration(2000)
      .tween("variableUpdate", function () {
        return tween(this.loc.y); // Pass the variable you want to update
      })
      .on("end", () => {
        console.log("Transition completed!");
        // Additional logic to handle transition completion
      });
  }

  // ------ // 

  updateVariable(startY, endY) {
    var interpolateLocY = d3.interpolate(startY, endY); //this.initialValue, this.targetValue);
   ...
   this.groupDOM.transition()
      ...
      .tween('custom-tween', () => {
        ...
        return (t) => {
          this.loc.y = interpolateLocY(t);
          //console.log ("loc y", this.loc.y)
          //this.createLinks( )
          this.drawLinks( ) // links outward
         _root.rows[0].drawLinks( ) // update previous block links (links inward)
          this.groupDOM.attr ('transform', 'translate('+this.loc.x+','+this.loc.y+')')
  ...
  drawBreadCrumbs( text) { 
  	this.groupDOM.append("text")
					  .text(text)
					  .attr("x", 120) // X coordinate of the text element
					  .attr("y", -5) // Y coordinate of the text element
...
export class BlockItem  { 
  ...
  constructor (data, dom, parent) { 
  	...
  	this.selectedColor = selectColour
		...
		//console.log ('--------------------')
				...
				this.domElement.selectAll ('#' + this.data.name + '_group')
						...
						.attr ('fill', (d, i) => { 
							...
							return 	i === 0 ? 'gray' : i === 1 ? this.selectedColor : i === 2 ? this.flowColor : undefined;
						})
						.attr ('opacity', (d, i)=> { 
							//return 0;
							//console.log ("active row =", _root.activeRow, ' this row ', this.parent.rowindex )
							//console.log ('this row = ', this.parent.rowindex)
							let alpha = this.parent.rowindex ==  _root.activeRow ?  0.9 : 0.3
							if (i == 2) alpha = 1;
							return alpha
  ...
  clickBlockItem (obj, e, d) { 
  		...
  		// --- clickable when block 'isActive' OR item 'isSelected'
			if (this.isActive ||  isSelected == 'true' )  {
					...
					this.parent.drawBreadCrumbs( this.data.name )

					_root.addToSelection_Sankey (selection, this); // -- perhaps this is where I build sankey from ? 
					_root.moveRows (this.parent.rowindex)

					// FIND THE CLUSTER and MAKERS in the LAYOUT (base and filter.. )
					// OR -- FIND MAKERS from SELCTED MAKERS 

					_root.findMakersInLayout (d.base); // base nodes 
...
drawLinks ( ) { 
	...
	if (this.parent.rowindex < _root.rows.length-1) { 
			let thisY = _root.rows[this.parent.rowindex].loc.y;
			let nextY = _root.rows[this.parent.rowindex+1].loc.y;
			ySpacing = nextY - thisY
			//console.log ('this y ', thisY, ' next y ', nextY)
	}
...
function createTween(initialValue, targetValue) {
  return function (variableToUpdate) {
    var i = d3.interpolate(initialValue, targetValue);
    return function (t) {
      variableToUpdate = i(t); // Update the variable directly
    };
  };
