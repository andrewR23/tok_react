	...
	// -- FORCE functions : set values / add all forces / remove all forces 
		...
		forcesTick ( ){ 
			...
			this.clusterlist.forEach ((c, i) => { 
					...
					c.moveToPos (this.clusterdata[i].x, this.clusterdata[i].y); // move using transition (inc delay)
...
// -- set MOVE FUNCTION -- to use for ALL -- 
	moveToPos (x, y) {
		...
		// 	//.duration (10)
		// 	.attr ('cx', d => { 
		// 		//console.log ('data attached to the group =', d)
		...
		// 	//.attr ('transform','translate('+this.loc.x+','+this.loc.y+')' )
 		...
 		// this.group data = cluster data.
	  // this group is the container for the shapes -- // 
		this.group
			//.transition ( )
			//.duration (2000)
			//.attr ('transform','translate('+this.tloc.x+','+this.tloc.y+')' )
