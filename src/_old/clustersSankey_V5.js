...
console.log ('cluster v5')

export class BlockClass  { 

	constructor (svg, groupdata, loc, i) { 
				this.svg = svg;
...
export class BlockItem  { 
  ...
  constructor (data, dom, parent) { 
  	...
  	this.nodeGroup = { selected: [ ], unselected: [ ], flow : [ ] }
  	...
  	this.domElement.selectAll ('#' + this.data.name)
		...
		// -- DRAW INDIVIDUAL ITEMS (as a sub-)
				...
				this.domElement.selectAll ('#' + this.data.name + '_group')
						...
						.attr ('fill', (d, i) => { 
							return  i == 0 ?  this.highlightCol : 'gray'
						})
						.attr ('opacity', (d, i)=> { 
							return  i == 0 ?  '0.9' : '0.3'
						})
						.attr ('isSelected', (d, i) => i == 0 ? true : false)
						// .on("click", function(d) {
    				// 					console.log(d);
						// });
						.on ('click',  (e, d) => { 
								//console.log ('source item = ', d, " ")
								//console.log (d3.select (this))
								//let data_nodes = d3.select (this)._groups[0][0].data;
								//console.log ('clicked nodes =', data_nodes)
  ...
  clickBlockItem (obj, e, d) { 
			// console.log ('clicked item', obj.data)
			// console.log (e, ' ', d);
			// console.log (e.target.getAttribute ('isSelected'))
