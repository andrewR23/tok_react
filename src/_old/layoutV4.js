 	...
 	// -- FORCE (NETWORK) LAYOUT -- // 
		if (this.type == 'force') { 
		  	...
		  	this.clusterlist.forEach ((c, i) => { 
		  		...
		  		this.clusterdata[i].size = this.clusterdata[i].nodes.length * 1 + 1; // reset cluster size data
	...
	setForces_layout ( ) { 
			...
			this.simulation
			    ...
			    .force ('collide',d3.forceCollide( ).radius((d, i) => {
			    			return d.size*1 })) //  this.forceCollide)
...
export class ClusterClass { 
	...
	drawShapeGroup(fill) { 
			...
			// -- as an END circle -- // 
			this.clusterShape = this.group.selectAll ('#clusterCircleEnd')
				.data (this.data)
				.join(
		      function(enter) {
		        return enter.append('circle')
