...
// -- 7. FILTER  LAYOUT - UPDATE SELECTION -- 
	filterLayout_date(date) { 
		// -- select ALL clusters -- 
		// -- filter layour 
		this.clusterlist.forEach ((c, i) => {
				let selection = c.filterByDate(date) ;// get a selection 
				let filterednodes = c.filterNodes (selection); // update 
				this.clusterdata[i].nodes = filterednodes; // [...this.nodes] ;// update nodes i cluster data
				this.clusterdata[i].size = filterednodes.length * 5 + 1; 
		})
...
export class ClusterClass { 
	...
	// -- draw group -- CONTAINER for cluster
	drawGroupElement ( ) { 
...
// -- UPDATE NODES by DATE -- // 
	 filterNodes (selection) { 
		 ...
		 this.restartForces_nodes( ); // restart forces -- 
