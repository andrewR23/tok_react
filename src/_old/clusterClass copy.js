...
// -- 4. SET LAYOUT with FORCES 
	...
	// -- create forces -- 
	// createForces_layout( ) { 
	// 		this.simulation.nodes (this.clusterdata); 
	...
	setForces_layout ( ) { 
			...
			//this.setForceValues( );
			this.simulation
					.force ('forceX', d3.forceX().x(this.center.x).strength(.01))// this.forceX)
			    .force ('forceY', d3.forceY().y(this.center.y).strength(.01)) //this.forceY)
...
// -- 7. UPDATE  / FILTER  LAYOUT - UPDATE SELECTION -- 
	filterLayout_date(date) { 
		// -- select ALL clusters -- 
		// -- filter layour 
		// this.clusterlist.forEach (c => {
		// 	let selection = c.filterByDate(date); // create selection -- 
		// 	 c.filterNodes (selection);					 	// update nodes data  --

		// })

		//console.log (date)
		// -- get selection for ONE cluster 
		let clusterInt = 0;
		let selection = this.clusterlist[0].filterByDate(date) ;// get a selection 
		//console.log ('selection', selection)
  	this.clusterlist[0].filterNodes (selection); // update 
  	//this.clusterlist[clusterInt].filterUpdate (selection); // update 
...
export class ClusterClass { 
	...
	// -- draw group -- placeholder for cluster
	drawGroupElement ( ) { 
...
// -- MOVE CLUSTER - --  by TRANSITION -
	setGroupXY_transition (x, y) { 
		// stop forces on nodes.-- // 
		...
		// --  move the group (Container) to location
		this.group
					.transition ( )
					.duration (4000)
		...
		// -- change color -- try a colour transition of cluster shape 
		this.clusterShape
				.transition()
			  .duration(1000)
			  .attr ("fill", 'pink')
		...
		// -- move nodes within group -- // 
		this.moveNodes_transition( );
