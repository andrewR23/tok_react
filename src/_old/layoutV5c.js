...
// -- 3. SET VISUALS : cluster shape / Size / Colour
	...
	// -- update scale for ALL clusters.. 

	// -- TESTS -- // 
			// updateScaleV1 (target) { 
			...
			// 	// ------------------- //
			// 	let scaleTarget = 0.1; // target; // 0.1; 
			// 	let startValue = this.clusterdata[0].scale
			...
			// 	let scaleInc = (scaleTarget - startValue) / stepCount; // 

			// 	function changeScale (layout)  { 
			// 		// -- update scale - towards target 
			// 		layout.clusterdata[0].scale += scaleInc; // 0.03; 			// scle on source data -- //  
			// 		//d.clusterlist[0].data[0].scale += 0.01;	// scale on cluster item -- // 

			// 		//  -- redraw -- 
			// 		 layout.setForces_layout( ); // apply forces to the whole of the clusters -- // 
			// 		 layout.setForces_nodes( )
			// 		 // layout.restartForces( );
			// 		 layout.drawClusters ( ); 
			// 		 layout.drawNodes ( );

			// 		// // -- stop running --  
			// 	 	if ((scaleInc > 0 && d.clusterdata[0].scale >= scaleTarget) || 
			// 	 		(scaleInc < 0 && d.clusterdata[0].scale <= scaleTarget)) {
		    // 				clearInterval(intervalId);
			// 		}
			
			// 	}
			// 	let intervalId = setInterval(( )=> {changeScale (this) }, interval);
			// 	//this.restartForces( );
			// }

			// // --------------------------------------// 

			// // update scale v2 --> get a sl
			// updateScaleV2 ( ) { 
			// 	let clustersToScaleUp = [this.clusterlist[0], this.clusterlist[1], this.clusterlist[2], this.clusterlist[3]]
			// 	let clustersToScaleDown = [this.clusterlist[5], this.clusterlist[6], this.clusterlist[7], this.clusterlist[9]]

			// 	let scaleUpTarget = 3; 
			// 	let scaleDownTarget = 0.1
			...
			// 	// -- interval function -- // 
			// 	function scaleItems (layout) {
			// 		// scale each cluster in list
			// 		clustersToScaleUp.forEach ((c,i)=> {
			// 			let scaleUpInc = (scaleUpTarget - startValue) / stepCount; // 


			// 			let isScaling = c.scaleCluster(2) ; // update scale. 
			// 			// -- when scaling is complete: remove from arry 
			// 			if (isScaling == false) {
			// 				clustersToScaleUp = clustersToScaleUp.filter (item => item !== c); // remove from array
			// 				if (clustersToScaleUp.length == 0) { clearInterval (intervalId)}
			// 			}
			// 		})
			// 		// update layout
			// 		 layout.setForces_layout( ); // apply forces to the whole of the clusters -- // 
			...
			// 	this.restartForces( ); // if forces top.. 
			// 	// ---- /// 
			// 	let intervalId = setInterval(( )=> { scaleItems(this) }, 50);
			// }
	// --------------------- // 
	...
	updateScale ( ) { 
		...
		function scaleItems (d) { 
			...
			d.itemsToScale.up.forEach (c => { 
				let isScaling = c.scaleCluster(1.5, 0.1) ; // update scale. 
...
export class ClusterClass { 
	...
	constructor (layout, dom, data){ 
		...
		// -- check the nodes and the links in this cluster -- 
			// console.log ('nodes in cluster =', this.data[0].nodes)
			// // get original makers (to check that there are links -- )
			// this.data[0].nodes.forEach ( n => { 
			// 	console.log ('maker =', _root.base_nodes.find (bn => bn.id == n.id))
			// })
			// console.log ('---------------------------')
	    // --------------------------------------------------- //
		...
		// -- link data 
		this.links_base = this.data[0].links; 
		this.links  = [...this.links_base]
...
// -- CLICK CLUSTER - interactions 
	...
	clickCluster ( ) {
		...
		//this.layout.updateSelectedClusters( ); // change colour
		//this.layout.updateScaleV2( );

		// this.layout.selectedItems.push (this) ;// add to selected
		// this.layout.itemsToScale.push (this) ; // add 'to scale'
		// this.layout.updateScale ( );
...
// -- APPLY / REMOVE FORCES to NODES --
	...
	applyForcesToNodes ( )  { 
			...
			this.nodesimulation
				// .force ('forceX', d3.forceX().x(0).strength(0.7))// this.forceX)
			  	// .force ('forceY', d3.forceY().y(0).strength(0.7)) //this.forceY)
			   	// .force ('repel',  d3.forceManyBody( ).strength(-200)) // this.forceCharge)
			    //.force ('collide',d3.forceCollide( ).radius(d => { return 10 * 1}).strength(0.9)) //  this.forceCollide)
			    .on ('tick', this.nodeForces_tick.bind (this))
	}
	// --------------------------------- //
	applyForcesToNodes_links ( ) { 
