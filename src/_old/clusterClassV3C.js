...
import * as _root from './mainV1.js'
...
// -- 2b. (RE) DRAW (update) LAYOUT -- 
	...
	// -- DATE LAYOUT -- // 
 	...
 	// -- FORCE (NETWORK) LAYOUT -- // 
		if (this.type == 'force') { 
		  	// -- calcuate (forces) posision-- // 
		  	this.setForces_layout( );
		  	this.restartForces( );

		  	// -- translate BACk -- (colour and size)
		  	// -- REset Colour and Shape 
		  	this.clusterlist.forEach ((c, i) => { 
		  			//console.log ('data = ', this.clusterdata[i].size)
		  		this.clusterdata[i].size = this.clusterdata[i].nodes.length * 5 + 1; // reset cluster size data
...
// -- FILTER NODES by DATE -- // 
	 ...
	 sortNodesByDate ( ) { 
			...
			console.log ('no of nodes in cluster = ', clusternodes.length)
