...
import * as clustersSankey from './clustersSankey_V4.js' // new version of clusters 
...
// -- 3. SET VISUALS : cluster shape / Size / Colour
	...
	updateSelection (cluster ) { 
		...
		//console.log ('clicked selection = ',  selection)

		//_root.showSelection (selection.map (n=> n.id))
	...
	drawShapeNodes(fill) { 
		...
		// -- draw the nodes -- // 
		 this.nodesGroup = this.group.selectAll ('#nodeCircle')
			...
			.attr ('fill', d => { 
						return this.selectedNodes.includes (d) ? 'orangered' : 'blue'
	  ...
	  // -- draw the links between nodes -- . 
			this.nodeLines = this.group.selectAll ('#nodeLine')
				...
				.attr ('id', 'nodeLine')
			 	.attr ('stroke',  d => { 
						return this.selectedLinks.includes (d) ? 'orangered' : 'blue'
				})
