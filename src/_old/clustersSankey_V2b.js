...
export class BlockItem  { 
  ...
  constructor (data, dom, parent) { 
  	...
  	//this.nodes = [...this.nodes_base]; // all nodes (ungrouped) ?? needed? 


  	// -- nodes put in groups
  	this.nodes_base_group = [[...this.nodes_base]] // base nodes [grouped] (remain unchanged)
  	this.data.nodes = [[...this.nodes_base]] ;// nodes (filtered )
  	...
  	//this.width = this.nodes.length * 5; // width of the block (total )/ 
  	this.width =  this.getNodeLength( ) * 5; // widths as total nodes 
  	//console.log ('nodes length  = ', this.nodes.length)
  	//console.log ("node sum = ", this.getNodeCount( ))

  	// --- //
  	this.splitGroupTest_Base_redo( ); // divide into two groups (by number.. )
  	//this.splitGroupTest_Base( )
  	//this.splitGroupByConditon( );
  	//this.drawGroup ( );


  	//this.width =  this.getNodeCount( ) * 5; // widths as total nodes 
  ...
  splitGroupTest_Base_redo ( ) { 
	  	...
	  	if (this.data.nodes[0].length > 4) {   		
	  		...
	  		this.data.nodes[0] = selection1; //  this.data.nodes[0].filter (n => selection.includes (n) == false)
	  		this.nodes_base_group[0] = [...selection1]; // this.nodes_base_group[0].filter (n => selection.includes (n) == false)
