...
import * as d3Class from './layoutV4.js'
...
// -- GET LISTS of the ATTRIBUTE NAME -- // 
...
// console.log(advInstruments);// 
// console.log(knownInstruments);// 
...
// -- sort into guild clusters.. -- // 
 //let guild_Clusters = getGuildClusters (base_nodes_copy) // getClusters_social(base_nodes_copy);

// sort into guilds - a test -- // 
...
//   const makersByGuild = base_nodes_copy.reduce((acc, maker) => {
//     if (maker.guilds.length === 0) {
//       if (!acc.none) {
//         acc.none = [maker.id];// if 'none' does not exist - add it 
//       } else {
//         acc.none.push(maker.id); // if 'none' exists - push maker into acc
//       }
//     } else {
...
// console.log (' makers by guild ', makersByGuild);
console.log ('guild_Clusters  ', guild_Clusters)
//console.log ('sorted makers by guild ', sortedMakersByGuild);
...
// console.log ('example ', social_Clusters[0]);

// attributes : name / index / nodes -- (nodes are the makers )
...
// -- SORT FUNCTIONS (put in a separate file ? )
