...
import * as clustersSankey from './clustersSankeyV2.js' // new version of clusters 
	...
	// -- DATE LAYOUT -- // 
		if (this.type == 'date') { 
				...
				this.clusterlist.forEach ((c, i) => { 
						...
						if (i == 0) x = -100 -_root.socialLayout.centre.loc[0] ; // include layout offset 
						if (i>0) { 
						...
						let y = this.dateScale (minDate) - _root.socialLayout.centre.loc[1] 
...
// -- 6. FILTER  LAYOUT - UPDATE SELECTION -- 
	...
	//----------------------------------- //
	filterLayout_selection (selection) { 
			this.clusterlist.forEach ((c, i) => {
		 	// each cluster needs its OWN slection 
			let clusterSelection = c.filterByID(selection) ;// get the items which are in this selection.. 
		

			// -- filter nodes -- hide all nodes 
			//let filterednodes = c.filterNodes (clusterSelection); // update nodes in cluster 
		 	//this.clusterdata[i].nodes = filterednodes; // [...this.nodes] ;// set nodes to new nodes
		 	// -- or -- filter clusters - hide cluster if they don't contain any elements.. 
		 	let filteredclusters = c.filterClusters (clusterSelection)
		 	this.clusterdata[i].nodes = filteredclusters
	...
	// --------------------------
	findLayoutItems_selection (selection)  { 
		this.clusterlist.forEach ((c, i) => {
			let clusterSelection = c.filterByID(selection) ; // find clusters (array)
			let nodeSelection = c.filterNodes (clusterSelection); // find nodes ? 
			//console.log ('cluster selection = == ', clusterSelection)
			//console.log ('node selection === ', nodeSelection);

			/// draw blue circle 
			if (clusterSelection.length> 0) { 
			 	console.log ('cluster = ', clusterSelection[0])
			// 	this.svg.append ('circle')
			// .attr ('cx', clusterSelection[0].parent_x + clusterSelection[0].x)
			// .attr ('cy', clusterSelection[0].parent_y + clusterSelection[0].y)
			// .attr ('r', 10)
			// .attr ('fill', 'blue')
			// .attr ('opacity', 0.4)
			 }
	...
	drawShapeNodes(fill) { 
		 ...
		 this.nodesGroup = this.group.selectAll ('#nodeCircle')
			...
			.attr ('r', d => { 
						//const r;// 5;
					 	...
					 	const r = this.flowNodes.includes(d) ? 5 :
              				    this.selectedNodes.includes(d) ? 4 : 3
              		return r
			})
			.attr ('fill', d => { 
						const color = this.flowNodes.includes(d) ? flowColor :
              						  this.selectedNodes.includes(d) ? selectColour : 'lightgray';
			...
			.attr ('opacity', d=> { 
						...
						const alpha = this.flowNodes.includes(d) ? 0.9:
              						  this.selectedNodes.includes(d) ? 0.3 : 1;
              		return alpha
	  ...
	  // -- draw the links between nodes -- . 
			this.nodeLines = this.group.selectAll ('#nodeLine')
				...
				.attr ('id', 'nodeLine')
			 	.attr ('stroke',  d => { 
						return this.selectedLinks.includes (d) ? 'red' : 'gray'
				})
...
// -- FILTER NODES by DATE -- // 
	 filterNodes (selection) { 
	 	//console.log ('selection  to filter = ', selection)
	 ...
	 filterClusters (selection) { 
	 	// see if ANY of selection is contained within cluster.. 
	 	const isInCluster = this.nodes.some(element => selection.includes(element));
	 	//console.log ('in this cluster ? ', isInCluster)

	 	if (isInCluster == false) this.nodes = [ ]; 
	 	if (isInCluster == false) this.links = [ ]; 
	 ...
	 // -- FILTER by ID ARRAY 
	 filterByID (idarray) {
	 	let idFilter = this.nodes_base.filter (n => idarray.includes (n.id))
		return idFilter; 
	 }
